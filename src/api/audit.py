...
async def get_audit_logs(
    ...
    # Build query
    query_parts = [f"organization_id=eq.{org_id}"]
...
async def get_audit_summary(org_id: str, request: Request):
    ...
    for log in logs:
        ...
        # Aggregate by user
        user_email = log.get("user_email", log["user_id"])
    ...
    total_result = await supabase.request(
        f"/audit_logs?organization_id=eq.{org_id}&select=id"
    )
    total = len(total_result.get("data", []))
...
async def export_audit_logs(
    ...
    if format == "csv":
        ...
        return Response(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=audit_logs_{org_id}_{datetime.now().strftime('%Y%m%d')}.csv"
            }
        )
