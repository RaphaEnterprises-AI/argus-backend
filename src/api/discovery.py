...
async def get_discovered_pages(
    ...
    return [
        DiscoveredPageResponse(
            ...
            elements_count=p.get("elements_count", 0),
            forms_count=p.get("forms_count", 0),
            links_count=p.get("links_count", 0),
            discovered_at=p.get("discovered_at", session["started_at"]),
...
async def update_flow(flow_id: str, request: UpdateFlowRequest):
    ...
    return DiscoveredFlowResponse(
        ...
        test_generated=flow.get("test_generated", False),
